...
@attr = { :name => @product.name, :image_url => @product.image_url, :id => @product.id, :price => "359" }
put :update, params: { :id => @product.id, :product => @attr }
@product.reload
expect(@product.price).to eq 359
end
end
...
before do
@product = FactoryGirl.create(:product)
@user = FactoryGirl.build(:admin) #oder @user = FactoryGirl.create(:user)
sign_in @user
end
